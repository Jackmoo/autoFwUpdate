...
if IS_UPDATE:
	print("choose: \n(1)to update latest DAILY firmware(NOT WORKING) \n(2)to update any latest firmware(NOT WORKING) \n(3)specify the firmware name(NOT WORKING) \n(4)specify fw with path under 192.168.76.211:/Firmware_Release/ES_daily_build/")
	updateDailyType = raw_input('Please choose(default is 4)')
	if updateDailyType == "3":
...
if IS_UPDATE:
	...
	sshProcess.expect('.fw', timeout=120)  #since the search takes long time, timeout set to 2min
	if updateDailyType == '4':
		absoluteUpdateFwPath = '/mnt/fwupdate'+updateFwPath
	else:
		updateFwPath = sshProcess.before
		absoluteUpdateFwPath = '/mnt/fwupdate'+updateFwPath[3:]+'.fw' # remove '.', add /mnt/fwupdate
	print('===================================')
